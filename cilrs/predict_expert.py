...
def test(data_loader, model, config, device):
    ...
    with torch.no_grad():
        ...
        for data in data_loader:
            ...
            command = data['command'].to(device, dtype=torch.float)
            ...
            steer, throttle, brake, _ = model(encoding, gt_velocity, command)#, seg_fronts[-1])
            ...
            postfix = OrderedDict([('te_str_m', score['str_metric'].avg),
...
model = CILRS(config, device).float().to(device)
