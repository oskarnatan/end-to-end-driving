...
class x11(nn.Module): #
    ...
    def __init__(self, config, device):#n_fmap, n_class=[23,10], n_wp=5, in_channel_dim=[3,2], spatial_dim=[240, 320], gpu_device=None): 
        ...
        self.necks_net = nn.Sequential( #inputnya dari 2 bottleneck
            ConvBNRelu(channelx=[2*n_fmap_b2[4][-1], n_fmap_b2[4][1]], kernelx=1, paddingx=0, stridex=1),
            ConvBNRelu(channelx=[n_fmap_b2[4][1], n_fmap_b2[4][1]], kernelx=2, paddingx=0, stridex=2),
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
            nn.Linear(n_fmap_b2[4][1], n_fmap_b2[4][0]),
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in):#, gt_ss):
        ...
        steer = self.relu(control_pred[:,0]) * 2 - 1. # convert from [0,1] to [-1,1]
        throttle = self.relu(control_pred[:,1]) * self.config.max_throttle
        brake = self.sigmoid(control_pred[:,2]) #brake: hard 1.0 or no 0.0
