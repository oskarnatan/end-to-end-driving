...
from v8.config import GlobalConfig
from v8.data import scale_and_crop_image, scale_and_crop_image_cv, rgb_to_depth, swap_RGB2BGR
...
class v8Agent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file):
		...
		self.net = v8(self.config, torch.device("cuda:0")).float().to(torch.device("cuda:0"))
	...
	def run_step(self, input_data, timestamp):
		...
		# forward pass
		pred_seg, pred_wp, pred_sc = self.net(self.input_buffer['rgb'], self.input_buffer['depth'], target_point, gt_velocity)
		# pred_sc = self.net.gen_top_view_sc(self.input_buffer['depth'][-1], pred_seg) #ingat, depth juga sequence, ambil yang terakhir
