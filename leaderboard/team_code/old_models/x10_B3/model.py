...
class x10_B3(nn.Module): #
    ...
    def __init__(self, config, device):#n_fmap, n_class=[23,10], n_wp=5, in_channel_dim=[3,2], spatial_dim=[240, 320], gpu_device=None): 
        ...
        self.RGB_encoder.classifier[1] = nn.Linear(n_fmap_b3[4][-1], 2, bias=True) #untuk mendeteksi redl dan stops #nn.Sequential() 
        kaiming_init_layer(self.RGB_encoder.classifier[1])
        ...
        #SC
        self.SC_encoder = models.efficientnet_b3(pretrained=False) #efficientnet_b0
        self.SC_encoder.features[0][0] = nn.Conv2d(config.n_class, n_fmap_b3[0][0], kernel_size=3, stride=2, padding=1, bias=False) #ganti input channel conv pertamanya, buat SC cloud
    ...
    def forward(self, rgb_f, depth_f, next_route, velo_in):#, gt_ss):
        ...
        redl_stops = self.sigmoid(self.RGB_encoder.classifier(self.flatten(self.RGB_encoder.avgpool(RGB_features8))))
    ...
    def mlp_pid_control(self, waypoints, velocity, mlp_steer, mlp_throttle, mlp_brake, redl, stops, ctrl_opt):
