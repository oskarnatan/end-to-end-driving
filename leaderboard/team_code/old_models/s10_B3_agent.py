...
from s10_B3.config import GlobalConfig
from s10_B3.data import scale_and_crop_image, scale_and_crop_image_cv, swap_RGB2BGR #rgb_to_depth
...
class s10_B3Agent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file):
		...
		self.net = s10_B3(self.config, torch.device("cuda:0")).float().to(torch.device("cuda:0"))
	...
	def run_step(self, input_data, timestamp):
		...
		# forward pass
		pred_seg, pred_wp, psteer, pthrottle, pbrake, predl, pstops = self.net(self.input_buffer['rgb'], target_point, gt_velocity) #self.input_buffer['depth'],  pred_sc
