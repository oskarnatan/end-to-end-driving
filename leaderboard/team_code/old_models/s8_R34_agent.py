...
from s8_R34.config import GlobalConfig
from s8_R34.data import scale_and_crop_image, scale_and_crop_image_cv, swap_RGB2BGR#, rgb_to_depth
...
class s8_R34Agent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file):
		...
		self.input_buffer = {'rgb': deque(), 'gps': deque(), 'thetas': deque()} #'rgb_left': deque(), 'rgb_right': deque(), 'rgb_rear': deque(), , 'depth': deque()

		self.config = GlobalConfig()
		self.net = s8_R34(self.config, torch.device("cuda:0")).float().to(torch.device("cuda:0"))
	...
	def run_step(self, input_data, timestamp):
		...
		# forward pass
		pred_seg, pred_wp, psteer, pthrottle, pbrake = self.net(self.input_buffer['rgb'], target_point, gt_velocity) #, self.input_buffer['depth'] , pred_sc
		...
		steer, throttle, brake, metadata = self.net.mlp_pid_control(pred_wp, gt_velocity, mlp_steer, mlp_throttle, mlp_brake, self.cw) #MIX MLP AND PID
	...
	def save(self, tick_data):
		...
		# Image.fromarray(swap_RGB2BGR(tick_data['depth'])).save(self.save_path / 'depth' / ('%04d.png' % frame))
	...
	def save2(self, ss):#, sc):
		...
		# Image.fromarray(imgx).save(self.save_path / 'segmentation' / ('%04d.png' % frame))
		# Image.fromarray(imgx2).save(self.save_path / 'semantic_cloud' / ('%04d.png' % frame))
		...
		imgx2 = cv2.circle(imgx2, (wp_nxr_frame[-1][0], wp_nxr_frame[-1][1]), radius=4, color=(255, 255, 255), thickness=2)
		"""

		cwd = os.getcwd()
		# print(cwd+'/'+os.environ['SAVE_PATH']+'/'+self.sstring+'/segmentation/%04d.png' % frame)
		cv2.imwrite(cwd+'/'+os.environ['SAVE_PATH']+'/'+self.sstring+'/segmentation/%04d.png' % frame, imgx) #cetak predicted segmentation
		# cv2.imwrite(cwd+'/'+os.environ['SAVE_PATH']+'/'+self.sstring+'/semantic_cloud/%04d.png' % frame, imgx2) #cetak predicted segmentation
