...
from x12.config import GlobalConfig
from x12.data import scale_and_crop_image, scale_and_crop_image_cv, rgb_to_depth, swap_RGB2BGR
from team_code.planner import RoutePlanner


SAVE_PATH = os.environ.get('SAVE_PATH', None)
CONTROL_OPTION = os.environ.get('CONTROL_OPTION', None)
TLS_OPTION = os.environ.get('TLS_OPTION', None)

def get_entry_point():
	return 'x12Agent'


class x12Agent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file):
		...
		self.net = x12(self.config, torch.device("cuda:0")).float().to(torch.device("cuda:0"))
	...
	def run_step(self, input_data, timestamp):
		...
		steer, throttle, brake, metadata = self.net.mlp_pid_control(pred_wp, gt_velocity, mlp_steer, mlp_throttle, mlp_brake, predl, pstops, CONTROL_OPTION, TLS_OPTION) #MIX MLP AND PID
